...
const BotsNProxies = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!botsFile || !proxiesFile) {
      toast.error(`${t("UPLOAD_BOTH_FILES")}`)
      return
    }

    const url = process.env.REACT_APP_API_URL + process.env.REACT_APP_API_UPLOAD_FILES_ENDPOINT;
    ...
    .then(res => {
      if (res?.data?.error) {
        toast.error(`${t("COULD_NOT_UPLOAD")}: ${res?.error || "Something went wrong"}`)
      } else {
    ...
    .catch(e => {
      toast.error(`${t("COULD_NOT_UPLOAD")}: ${e.message || "Something went wrong"}`)
    })
  ...
  return (
    <div>
      <form style={{fontSize: '15pt', textAlign: 'center', margin: '0 auto', width: '700px'}} onSubmit={handleSubmit}>
        <div style={{textAlign: 'left', display: 'flex', width: '100%'}}>
          <div style={{color: "#fff", fontWeight: 'light', width: '400px'}}>{t('UPLOAD_BOTS')}</div>
          <input type="file" onChange={handleBotsChange} />
        </div>
        <div style={{textAlign: 'left', display: 'flex', width: '100%'}}>
          <div style={{color: "#fff", fontWeight: 'light', width: '400px'}}>{t('UPLOAD_PROXIES')}</div>
          <input type="file" onChange={handleProxiesChange} />
        </div>
        
        
        <button className='form-btn' type="submit">{t('UPLOAD')}</button>
      </form>
    </div>
  );
